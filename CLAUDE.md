...
- **Crawler events are self-reported.** The middleware sends `bot.ip` and `bot.userAgent` in the request body and the Worker writes them verbatim, unlike browser rows whose IP comes from `CF-Connecting-IP`. Origin is the only gate and is forgeable off-browser, so a forged crawl row can assert any IP, name and category. Matters only if bot IPs are ever treated as evidence.

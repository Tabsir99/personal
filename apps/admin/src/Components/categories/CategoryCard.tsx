  ...
  return (
    <Card
      className={`bg-zinc-900 border-zinc-800 text-white shadow-lg border-2 transition-all duration-300 flex flex-col justify-between
        ${editing.editing ? "max-h-[500px]" : "max-h-80"}`}
    >
      <CardHeader className="transition-all duration-300 ease-in-out overflow-hidden">
        <div
          className={`transition-opacity flex flex-col gap-3 duration-300 ease-in-out ${editing.editing ? "opacity-0 h-0" : "opacity-100 h-32"}`}
        >
      ...
      <CardContent className="flex flex-col justify-between transition-all duration-300 ease-in-out pb-6">
        <div className="space-y-3 text-xs uppercase tracking-wider">
          ...
          <div className="flex gap-3">
            ...
            <span className="text-zinc-300">{Category.totalPosts}</span>
          </div>
        </div>
      </CardContent>
      <CardFooter className="border-t border-zinc-700 h-14 flex justify-between items-center py-0 transition-all duration-300 ease-in-out">
        ...
        <div className="relative w-full flex justify-end">
          <div
            className={`absolute top-0 right-0 w-full flex justify-end transition-opacity duration-300 ease-in-out ${editing.editing ? "opacity-100 z-10" : "opacity-0 z-0"}`}
          >
            <SaveEdit
          ...
          <div
            className={`transition-opacity duration-300 ease-in-out ${editing.editing ? "opacity-0 z-0" : "opacity-100 z-10"}`}
          >
            <div className="flex space-x-1">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => setEditing({ ...editing, editing: true })}
      ...
      <AlertDialog open={editing.deleting}>

...
describeMaybe("generateBlogDraftContent — real Anthropic call", () => {
  it("produces a structured essay draft for a real topic", async () => {
    const topic =
    ...
    const rootTypes = (ai.doc.content as Array<{ type: string }>).map(
      (b) => b.type,
    );
...
describe.skipIf(RUN)(
  "generateBlogDraftContent — skipped without auth token",
  () => {

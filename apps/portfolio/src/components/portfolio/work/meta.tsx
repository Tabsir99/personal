    ...
    >
      {projects.map((project, i) => (
        <div
          ...
          className="work-meta grid grid-cols-[0.78fr_1.22fr] gap-10 items-start max-xl:grid-cols-1 max-xl:gap-8"
        >
          <div>
            ...
            <div className="flex gap-2">
              {project.links.map((l, j) => {
                const className = cn(
                  "group/link inline-flex mt-6 items-center gap-2.5 px-4 py-2.5 border border-line rounded-xs font-mono text-xs tracking-widest uppercase text-cream-2 bg-transparent transition-all duration-250 ease-out hover:border-accent hover:text-accent hover:-translate-y-px",
                  LINK_BG[l.type],
                );
                const inner = (
                  <>

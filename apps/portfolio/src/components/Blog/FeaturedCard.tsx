...
export default function FeaturedCard({ post }: { post: PostMeta }) {
  return (
    <article
      data-reveal
      className="relative bg-ink-2 border border-line rounded-[18px] overflow-hidden grid grid-cols-[1.05fr_1fr] mb-20 transition-transform hover:-translate-y-1.5"
    >
      <div
      ...
      <div className="py-10 px-11 flex flex-col gap-5">
        ...
        <Link
          className="group inline-flex items-center gap-3.5 self-start mt-4 px-[22px] py-3.5 bg-cream text-ink rounded-full font-bold text-sm transition overflow-hidden hover:bg-accent hover:translate-x-1"
          href={`/blog/${post.slug}`}
        >
